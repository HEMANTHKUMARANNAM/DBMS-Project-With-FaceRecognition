...
app.post('/facedata', (req, res) => {
    const obj = {};
    console.log("server called");

    const childPython = spawn('python', ['face_recog.py', JSON.stringify(obj)]);
...
app.post('/saveData', (req, res) => {
    ...
    fs.writeFile('TEACHERS/data.json', jsonString, (err) => {
        if (err) {
            console.error('Error writing to file:', err);
            res.status(500).send('Error writing data to file');
        } else {
            console.log('Data saved to file:', jsonString);
